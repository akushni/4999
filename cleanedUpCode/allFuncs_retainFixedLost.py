...
def simulation(N, t, numTrajs, p0, sType, c = None, d = None, noiseSTD = None):
    ...
    if sType == neutral:
        ...
        for i in range(degree): # draw weights from geometric distribution with p = 0.? and centered at 0
            if c < 0:
                coeffs[i] = ((-1)**(i+1))*sp.stats.geom.pmf(i, abs(c), -1) 
            else:
                coeffs[i] = ((-1)**(i))*sp.stats.geom.pmf(i, c, -1) 
        # coeffs = np.array([c**i for i in range(1,d+1)])
...
def procWithFT(trajectories, numTrajs, t):
    ...
    ######################
        counter = 0
        for j in range(t+1):
            if trajectories[j,i] != 1 and trajectories[j,i] != 0:
                counter += 1
                
        shuffledIncs[0:counter,i] = np.random.permutation(increments[0:counter,i])
        shuffledIncs[counter:,i] = 0
        
        FisherIncsShuffled[0:counter,i] = np.random.permutation(FisherIncrements[0:counter,i])
        FisherIncsShuffled[counter:,i] = 0
    #######################  
...
def procWithVarT(trajectories, numTrajs, t):
    ...
    shuffledIncs = np.zeros((t, numTrajs))
    for i in range(numTrajs):  # for each traj.
        if (trajectories[-1,i] != 1) and (trajectories[-1,i] != 0): # only permuting not fixed/lost trajectories
            ...
            shuffledIncs[:,i] = increments[:,i]
            transformedIncsShuffled[:,i] = transformedIncs[:,i]
